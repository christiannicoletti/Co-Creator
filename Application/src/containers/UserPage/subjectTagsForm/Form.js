...
class userform extends Component {
  ...
  submitHandler = event => {
    ...
    if (this.state.subjectTags.information.value !== 'Choose subjects...') {
      this.props.submitSubjectTagsForm(
        username,
        this.state.subjectTags.information.value
      );
    }
  };
  ...
  inputChangedHandler = (event, inputIdentifier) => {
    ...
    const updatedFormElement = updateObject(
      this.state.subjectTags[inputIdentifier],
      {
        value: event.target.value,
        valid: checkValidity(
          event.target.value,
          this.state.subjectTags[inputIdentifier].validation
        ),
        touched: true
      }
    );
    ...
    // Updating userForm
    const updatedSubjectTags = updateObject(this.state.subjectTags, {
      [inputIdentifier]: updatedFormElement
    });

    // Checking if all of form is valid
    let formIsValid = true;
    for (let inputIdentifier in updatedSubjectTags) {
      formIsValid = updatedSubjectTags[inputIdentifier].valid[0] && formIsValid;
    }

    this.checkResetHandler(event.target.value);

    // Setting new state
    this.setState({ subjectTags: updatedSubjectTags, formIsValid: formIsValid });
  };
  ...
  render() {
    ...
    for (let key in this.state.subjectTags) {
      formElementsArray.push({
        id: key,
        config: this.state.subjectTags[key]
      });
    }
        ...
        <form onSubmit={this.submitHandler}>
          ...
          <div className={classes.ButtonsContainer}>
            ...
            <Button
              ...
              clicked={this.props.switchSubjectTagsForm}
            />
          </div>
        </form>
      </WithClass>
...
const mapStateToProps = state => {
  return {
    postSubjectTagsComplete: state.userProfilePost.postSubjectTagsComplete
  };
};

const mapDispatchToProps = dispatch => {
  return {
    submitSubjectTagsForm: (username, text) => {
      dispatch(actions.postSubjectTags(username, text));
    }
  };
};
